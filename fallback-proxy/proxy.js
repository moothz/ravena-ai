...
function serveFallback(req, res, err) {
    ...
    const isApi = pathname.startsWith('/api/') || (req.headers.accept && req.headers.accept.includes('application/json'));
    if (isApi) {
        res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8', 'X-Ravena-Status': 'offline' });
...
server.listen(PROXY_PORT, '0.0.0.0', () => {
    console.log(`[Ravena Fallback Proxy] Running on port ${PROXY_PORT}, proxying to 127.0.0.1:${TARGET_PORT}`);
});
