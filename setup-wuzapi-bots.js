...
const whatsappBots = bots.filter(
  (b) => b.enabled && !b.useDiscord && !b.useTelegram
);
...
async function modeQr() {
  ...
  for (const bot of whatsappBots) {
    ...
    // Busca QR code
    const result = await wuzapiRequest("GET", `/api/get-qr?name=${encodeURIComponent(instanceName)}`).catch(() => null);
...
async function modeWebhook() {
  console.log("\n🪝 Modo: WEBHOOK — Configurando webhooks\n");

  for (const bot of whatsappBots) {
    const instanceName = bot.nome;
    const webhookPort = bot.webhookPort || parseInt(API_PORT);
    const webhookSecret = bot.webhookSecret || generateSecret();

    const webhookUrl = `${HOSTNAME}:${webhookPort}/wuzapi/webhook/${encodeURIComponent(instanceName)}`;

    console.log(`\n🔹 Instância: ${instanceName}`);
    console.log(`  URL: http://${webhookUrl}`);

    // Configura webhook por instância
    const result = await wuzapiRequest("POST", `/api/set-webhook`, {
      name: instanceName,
    ...
    if (result?.status === 200) {
      ...
      // Salva o webhookSecret no bots.json se não existia
      if (!bot.webhookSecret) {
        bot.webhookSecret = webhookSecret;
        saveBots();
      }
    } else {
      console.error("  ❌ Falha ao configurar webhook:", result);
    }
  }
